...
class CircularQueue
  ...
  def enqueue(obj)
    if buffer.all? { |item| item == nil }
      buffer[0] = obj
      order << 0
    elsif buffer.any? { |item| item == nil }
      pos = (order.last + 1) % buffer.length
      buffer[pos] = obj
      order << pos
    else
      buffer[order.first] = obj
      removed = order.shift
      order << removed
    end
  end
  ...
  def dequeue
    ...
    removed = buffer[order.first]
    buffer[order.first] = nil
    order.shift
    removed
  end
end
